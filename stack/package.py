...
class Package:
    ...
    def __package_base_args__(self) -> list:
        ...
        return [
            ...
            f"-n {self.C.get_key('product')}",
            "--provides redis",
            "--provides redis-server",
            f"--architecture {self.ARCH}",
            f"--vendor '{self.C.get_key('vendor')}'",
            f"--version {self.C.get_key('version')}",
            f"--url 'https://redistack.io'",
            f"--license {self.C.get_key('license')}",
            f"--category server",
            f"--maintainer '{self.C.get_key('email')}'",
            f"--description '{self.C.get_key('description')}]'",
    ...
    def package(
        ...
        if package_type == "deb":
            ...
            fpmargs.append(
                f"-p {self.C.get_key('product')}-{self.C.get_key('version')}-{build_number}.{distribution}.{self.ARCH}.deb"
            )
            ...
            fpmargs.append(
                f"-p {self.C.get_key('product')}-{self.C.get_key('version')}-{build_number}.{distribution}.{self.ARCH}.rpm"
            )
            ...
            fpmargs.append(
                f"-p {self.C.get_key('product')}-{self.C.get_key('version')}-{build_number}.{distribution}.osxpkg"
            )
            fpmargs.append("-t osxpkg")
        elif package_type == "pacman":
            fpmargs.append(
                f"-p {self.C.get_key('product')}-{self.C.get_key('version')}-{build_number}.{self.ARCH}.pacman"
            )
            fpmargs.append(
                f"--after-install {os.path.join(self.__PATHS__.SCRIPTDIR, 'package', 'postinstall')}"
            )
        ...
        #     fpmargs.append(
        #         f"-p {self.C.get_key('product')}-{self.C.get_key('version')}-{build_number}.{self.ARCH}.snap"
        #     )
